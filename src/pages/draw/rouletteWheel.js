...
class RouletteWheel extends Component {
  constructor(props) {
    ...
    const data = newResults.map((a) => {
      return { option: a.userName, optionSize: a.count };
    });

    const prizeNumber = data.findIndex((a) => a.option === winner);

    console.log('prizeNumber', prizeNumber);
    console.log('data', data);
    console.log('newResults', newResults);
    console.log('results', results);
    console.log('tickets', props.prize.tickets);
          ...
          {this.state.display && (
            <WheelDiv>
              <Wheel
                ...
                onStopSpinning={() => {
                  this.setState({ ...this.state, mustSpin: false });
                }}
                innerRadius={1}
                radiusLineWidth={2}
                fontSize={10}
                textDistance={75}
                backgroundColors={['#1ABC9C', '#CD5C5C']}
              />
            </WheelDiv>
          )}
        </ModalDiv>
